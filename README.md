...
## Methodology:
&nbsp;&nbsp;&nbsp;&nbsp;**Choosing a system:** Several methods using an AI and a software for CAD were considered to find the most efficient way to help new users. This includes pairing softwares such as Fusion, OnShape, AutoCAD, SketchUP, and Solidworks, and AI’s like Gemini and Copilot. The method chosen for this project was to use the capabilities of ChatGPT, an AI, with Blender, a CAD and modeling software, since both are very accessible, creating a flexible approach for beginners to engage with CAD design. Unlike AI-integrated CAD platforms that have subscription fees or require specialized knowledge, this approach combines Blender, an open-source 3D modeling tool, with ChatGPT, a widely accessible AI chatbot, making it easier for anyone to use without additional costs or learning curves. By using ChatGPT as a tutor, users can receive guidance, make code, ask AI about errors, and gain more of an understanding for scripting logic with Blender's python API. The upside is that all of this requires no prior programming experience. However, the downside is that the AI might not always provide perfect answers, and users might still need to troubleshoot on their own. In some cases, the AI may be able to respond to these issues, given specific information, such as the version or what the interface looks like.
